// src/components/YourPath/ManagerFinalRedefTask.js
import Swal from "sweetalert2";
import withReactContent from "sweetalert2-react-content";
import { supabase } from "../../supabaseClient";
import { ManagerFinalRedefData } from "./ManagerFinalRedefData";


const MySwal = withReactContent(Swal);

export const openCreateFinalRedefTask = async () => {
  ...
  const { value: formData } = await MySwal.fire({
    title: `<div style="color:#3B0304; font-weight:600; display:flex; align-items:center; gap:8px;">
      <i class="bi bi-list-check"></i> Create Final Re-Defense Task</div>`,
    width: "900px",
      ...
      <div style="display:grid; grid-template-columns: repeat(3,1fr); gap:12px; width:100%;">
        <div>
          ...
          <select id="methodology" class="form-select" disabled>
            ${Object.keys(ManagerFinalRedefData)
              .map(
                (m) =>
    ...
    didOpen: () => {
      ...
      methodologySelect.addEventListener("change", () => {
        ...
        if (ManagerFinalRedefData[selected]) {
          populateOptions(projectPhaseSelect, ManagerFinalRedefData[selected].phases, "Select Project Phase");
        }
      });
      ...
      projectPhaseSelect.addEventListener("change", () => {
        ...
        resetSelect(subtaskSelect, "");
        resetSelect(elementSelect, "");
        if (ManagerFinalRedefData[m]?.taskTypes[p]) {
          populateOptions(taskTypeSelect, ManagerFinalRedefData[m].taskTypes[p], "Select Task Type");
        }
      });
      ...
      taskTypeSelect.addEventListener("change", () => {
        ...
        resetSelect(subtaskSelect, "");
        resetSelect(elementSelect, "");
        if (ManagerFinalRedefData[m]?.tasks[tt]) {
          populateOptions(taskSelect, ManagerFinalRedefData[m].tasks[tt], "Select Task");
        }
      });
      ...
      taskSelect.addEventListener("change", () => {
        ...
        resetSelect(subtaskSelect, "");
        resetSelect(elementSelect, "");
        if (ManagerFinalRedefData[m]?.subtasks[t]) {
          populateOptions(subtaskSelect, ManagerFinalRedefData[m].subtasks[t], "Select Subtask");
        }
      });
      ...
      subtaskSelect.addEventListener("change", () => {
        ...
        resetSelect(elementSelect, "");
        if (ManagerFinalRedefData[m]?.elements[st]) {
          populateOptions(elementSelect, ManagerFinalRedefData[m].elements[st], "Select Element");
        }
      });
      ...
      window.__finalRedefSelectedMembers = selectedMembers;
    ...
    preConfirm: () => {
      ...
      const assignedMembers = (window.__finalRedefSelectedMembers || []).map((m) => m.id);
  ...
  // 🔹 Step 6: Insert to DB (manager_final_redef)
  if (formData) {
    ...
    if (error) {
      console.error(error);
      Swal.fire("Error", "Failed to create final re-defense task.", "error");
      return;
    }

    Swal.fire("Success", "Final Re-Defense Task Created!", "success");
    return data;
  }
};
