 ...
 const adviserIdInput = popup.querySelector("#user_id");
  ...
  adviserIdInput.addEventListener("input", (e) => {
    ...
    if (value !== cleanedValue) {
      ...
      validationMessage.textContent = "Only numbers are allowed for Adviser ID.";
    ...
    if (cleanedValue.length > 9) {
      ...
      validationMessage.textContent = "Adviser ID can only be 9 digits long.";
      validationMessage.style.display = "block";
    }
  });
...
var maxFutureYears = 1; // You can adjust how many future years to include
const yearOptions = [];
...
try {
  ...
  MySwal.fire("Success", `Advisers enrolled for ${selectedYear}!`, "success");
  setImportedData([]);
  setSelectedRows([]);
} catch (err) {
