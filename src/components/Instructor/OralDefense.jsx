...
const OralDefense = () => {
  ...
  const [advisers, setAdvisers] = useState([]); // panel pool (roles 3/4/5)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (accData) {
        setAccounts(accData);
        // panel pool includes: advisers (3), chairs (4) if any, and guests (5)
        const panelPool = accData.filter((a) =>
          [3, 4, 5].includes(a.user_roles)
        );
        setAdvisers(panelPool);
      }
  ...
  const sortedSchedules = [...schedules].sort((a, b) => {
    const priorityOrder = { 1: 1, 3: 2, 2: 3, 4: 4 };
  ...
  // team already scheduled?
  const isTeamAlreadyScheduled = (teamName) => {
    return schedules.some(
      (schedule) => schedule.manager?.group_name === teamName
    );
  };

  // list of already scheduled team names
  const getScheduledTeams = () => {
    return schedules
      .map((schedule) => schedule.manager?.group_name)
      .filter(Boolean);
  };

  // conflicts: same date and within 1 hour
    ...
    MySwal.fire({
      ...
      html: `
        <style>
          ...
          .disabled-option { color: #999; background-color: #f5f5f5; }
          .time-conflict-warning {
            color: #d63384; font-size: 12px; margin-top: 5px; font-weight: 500;
          }
        </style>
        <div class="mb-3">
      ...
      didOpen: () => {
        ...
        // set min date to today + open picker
        if (scheduleDate) {
          ...
          scheduleDate.setAttribute("min", `${yyyy}-${mm}-${dd}`);
          const openPicker = () =>
        ...
        // time min enforcement when date is today
        if (scheduleTime) {
          const openTimePicker = () =>
            scheduleTime.showPicker && scheduleTime.showPicker();
          scheduleTime.addEventListener("click", openTimePicker);
          scheduleTime.addEventListener("focus", openTimePicker);
          ...
          const setTimeMinIfToday = () => {
            ...
            if (picked.getTime() === today.getTime()) {
              ...
              scheduleTime.setAttribute("min", min);
          ...
          scheduleDate.addEventListener("change", setTimeMinIfToday);
        ...
        const checkTimeConflict = () => {
          ...
          if (date && time) {
            if (hasTimeConflict(date, time)) {
              ...
              timeConflictWarning.innerHTML = `⚠️ Time conflict with: ${text}. Please choose a different time with at least 1 hour gap.`;
            } else {
        ...
        // get already scheduled teams to mark in UI
        const scheduledTeams = getScheduledTeams();

        // populate teams for chosen adviser.group
        const updateTeamOptions = (adviserId) => {
          ...
          if (teams.length === 0) {
            ...
            teams.forEach((t) => {
              ...
              opt.textContent =
                t.group_name +
                (isAlreadyScheduled ? " (Already Scheduled)" : "");
        ...
        // build panel options: label guests, disable same-group non-guests; exclude current adviser from panel pool
        const rebuildPanelOptions = (teamAdviserGroup, excludeId) => {
          panelSelect.innerHTML =
            '<option disabled selected value="">Select</option>';
          advisers.forEach((p) => {
            // never list the selected adviser as panelist choice
            ...
            const sameGroup =
              teamAdviserGroup &&
              p.user_roles !== 5 &&
              p.adviser_group === teamAdviserGroup;

            const opt = document.createElement("option");
            opt.value = p.id;
            opt.textContent = `${p.last_name}, ${p.first_name}${
              isGuest ? " (Guest Panelist)" : ""
            }`;
        ...
        adviserSelect.addEventListener("change", () => {
          ...
          // enable team/panel after adviser is chosen
          ...
          // initial panel options (no team context yet: do not disable same-group, but still exclude adviser)
          rebuildPanelOptions(null, selectedAdviser);
        });
        ...
        teamSelect.addEventListener("change", () => {
          ...
          // reset panelists when team changes
          selectedPanelists = [];
          panelList.innerHTML =
            '<span class="text-muted">No panelist selected</span>';

          // now we know the team group — disable regular advisers from same group; guests are always allowed
          rebuildPanelOptions(teamAdviserGroup, selectedAdviser);
        });
      ...
      preConfirm: () => {
        ...
        if (hasTimeConflict(date, time)) {
          ...
          const conflictText = conflicts
            ...
            .join(", ");
          MySwal.showValidationMessage(
            `Time conflict detected with: ${conflictText}. Please choose a different time with at least 1 hour gap.`
          );
          return false;
        }
  ...
  const handleUpdate = (id) => {
    ...
    MySwal.fire({
      ...
      didOpen: () => {
        ...
        // date min today, time min when today
        if (scheduleDate) {
          const today = new Date();
          today.setHours(0, 0, 0, 0);
          const yyyy = today.getFullYear();
        ...
        // conflict warning live update (excluding this schedule)
        const checkTimeConflict = () => {
          const date = scheduleDate.value;
          const time = scheduleTime.value;
          if (date && time) {
            if (hasTimeConflict(date, time, schedule.id)) {
              ...
              const txt = conflicts
                ...
                .join(", ");
              timeConflictWarning.style.display = "block";
              timeConflictWarning.innerHTML = `⚠️ Time conflict with: ${txt}. Please choose a different time with at least 1 hour gap.`;
            } else {
              timeConflictWarning.style.display = "none";
            }
          } else {
        ...
        // figure team group to disable regular advisers from same group
        const teamName = schedule.manager?.group_name || null;
        const teamMembers = accounts.filter((a) => a.group_name === teamName);
        const teamAdviserGroup = teamMembers[0]?.adviser_group || null;

        // build options (guests labeled, same-group non-guests disabled, current adviser disabled, already selected disabled)
        const rebuildUpdateOptions = () => {
          panelSelect.innerHTML =
            '<option disabled selected value="">Select Panelist</option>';
          advisers.forEach((p) => {
            const isGuest = p.user_roles === 5;
            const isCurrentAdviser = p.id === schedule.adviser_id;
            const isAlreadyPanelist = selectedPanelists.includes(p.id);
            const sameGroup =
              teamAdviserGroup &&
              p.user_roles !== 5 &&
              p.adviser_group === teamAdviserGroup;
            ...
            let note = "";
            if (isCurrentAdviser) note = " (Current Adviser)";
            else if (isAlreadyPanelist) note = " (Already Selected)";
            else if (sameGroup) note = " (Team Adviser)";

            opt.textContent = `${p.last_name}, ${p.first_name}${
              isGuest ? " (Guest Panelist)" : ""
            }${note}`;

            if (isCurrentAdviser || isAlreadyPanelist || sameGroup) {
              opt.disabled = true;
              opt.className = "disabled-option";
            }
        ...
        panelSelect.addEventListener("change", () => {
          ...
          if (selectedOption.disabled) {
            MySwal.showValidationMessage(
              "This adviser cannot be selected as a panelist."
            );
            panelSelect.value = "";
            return;
          }
          ...
          if (!selectedPanelists.includes(id)) {
            if (selectedPanelists.length < 3) {
              ...
              // refresh disabled/enabled states
              rebuildUpdateOptions();
            } else {
              MySwal.showValidationMessage("Maximum of 3 panelists.");
            }
        ...
        panelList.addEventListener("click", (e) => {
          if (e.target.classList.contains("remove-panelist-btn")) {
            ...
            // refresh options to re-enable removed person
            rebuildUpdateOptions();
            ...
            {filteredSchedules.map((s, index) => (
              <tr key={s.id} className="relative">
                ...
                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-600 text-center">
                  <select
                    className={`w-32 p-1 border rounded-lg text-xs transition-colors ${getSelectStyle(
                      s.verdict
                    )}`}
                    value={s.verdict}
                    onChange={(e) =>
