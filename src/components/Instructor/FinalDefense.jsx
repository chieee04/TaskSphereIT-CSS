...
const FinalDefense = () => {
    ...
    // Sort schedules by verdict priority: Pending -> Re-Oral -> Not-Accepted -> Accepted
    const sortedSchedules = [...schedules].sort((a, b) => {
        const priorityOrder = {1: 1, 2: 2, 3: 3, 4: 4}; // Pending -> Re-Oral -> Not-Accepted -> Accepted
            ...
            html: `
                <div style="text-align: left;">
                    ...
                    <select id="exportFilter" class="form-select" style="border-radius: 8px; height: 42px; width: 100%;">
                        ...
                        <option value="2">Re-Oral Only</option>
                        <option value="3">Not-Accepted Only</option>
                        <option value="4">Accepted Only</option>
                    </select>
                </div>
            `,
            icon: "question",
            ...
            if (result.isConfirmed) {
                ...
                if (exportData.length === 0) {
                    const filterText = exportFilter === 'all' ? 'schedules' : 
                                    ...
                                    exportFilter === '2' ? 'Re-Oral schedules' : 
                                    exportFilter === '3' ? 'Not-Accepted schedules' : 'Accepted schedules';
    ...
    const handleVerdictChange = async (id, newVerdict) => {
        ...
        if (!error) {
            ...
            // Show success message for Re-Oral
            if (newVerdict === 2) {
                MySwal.fire({
                    icon: "success",
                    title: "Marked as Re-Oral",
                    text: "The schedule has been marked for re-defense and will remain in the list.",
                    showConfirmButton: false,
                    timer: 2000,
                });
            }
