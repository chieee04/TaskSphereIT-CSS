...
const secondYear = parseInt(selectedYear) + 1;
  ...
  const handleDeleteSelected = async () => {
    if (selectedRows.length === 0) {
      MySwal.fire({
        ...
        text: "Please select at least one adviser to delete.",
        icon: "info",
    ...
    const result = await MySwal.fire({
      title: `Delete ${selectedRows.length} Adviser${selectedRows.length > 1 ? 's' : ''}?`,
      text: "This will permanently delete the selected adviser(s).",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3B0304",
      cancelButtonColor: "#999",
      confirmButtonText: `Yes, delete ${selectedRows.length} adviser${selectedRows.length > 1 ? 's' : ''}!`,
    });
    ...
    if (result.isConfirmed) {
      try {
        ...
        // Remove deleted advisers from local state
        ...
        MySwal.fire("Deleted!", `${selectedRows.length} adviser${selectedRows.length > 1 ? 's' : ''} have been deleted.`, "success");
      } catch (error) {
        console.error("Delete error:", error);
        MySwal.fire("Error", "Failed to delete advisers.", "error");
      }
    }
  };
  ...
  const handleResetAllPasswords = () => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        try {
          // Generate new random passwords for all advisers
          const updates = credentials.map(adviser => ({
            id: adviser.id,
          ...
          // Update local state
          const updatedCredentials = credentials.map(adviser => ({
            ...adviser,
            password: updates.find(u => u.id === adviser.id)?.password || adviser.password
          }));
          setCredentials(updatedCredentials);
