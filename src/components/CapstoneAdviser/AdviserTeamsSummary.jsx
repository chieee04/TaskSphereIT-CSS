// src/pages/AdviserTeamsSummary.jsx  (or wherever you keep it)
...
const AdviserTeamsSummary = () => {
  ...
  const [members, setMembers] = useState([]); // [{first_name,last_name,role}]
  ...
  const [tasks, setTasks] = useState([]); // normalized
    ...
    (async () => {
      try {
        ...
        for (const g of [...groups].sort((a, b) => a.localeCompare(b))) {
          // small footer preview (e.g., "Mendoza, Et Al")
          let preview = "Team";
        ...
        if (data) {
          data.forEach((row) => {
            ...
            merged.push({
              ...
              elements: row.elements || "-", // if your schema has 'elements'
...
export default AdviserTeamsSummary;
