...
export default function AdviserFinalRecord() {
  ...
  // ✅ Fetch completed tasks from final_def table with team names
  const fetchCompletedTasks = async () => {
    try {
      // First, get all completed tasks from final_def table
      ...
      if (!finalDefTasks || finalDefTasks.length === 0) {
        setTasks([]);
        return;
      }

      // Get team names from groups table using manager_id
      const managerIds = [...new Set(finalDefTasks.map(task => task.manager_id).filter(Boolean))];
      ...
      const tasksWithTeamNames = finalDefTasks.map(task => ({
        ...task,
        ...
        html: `
          <div style="text-align: left; font-size: 0.95rem;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
              ...
              <div>
                <strong>Comment:</strong><br>${task.comment || "-"}
              </div>
            </div>
          </div>
        `,
  ...
  const renderTable = () => {
    if (currentView === 0) {
        ...
        <table className="tasks-table">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredAndSearchedTasks.length > 0 ? (
              ...
              <tr>
                <td colSpan={isSelectionMode ? 9 : 8} className="center-text" style={{ padding: '20px' }}>
      ...
      // Second View: Time, Date Completed, Revision No., Status, Methodology, Project Phase, Comment, Action
      return (
        <table className="tasks-table">
          <thead className="bg-gray-50">
            <tr>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredAndSearchedTasks.length > 0 ? (
              ...
              <tr>
                <td colSpan="9" className="center-text" style={{ padding: '20px' }}>
                  No completed tasks found
                </td>
              </tr>
            )}
