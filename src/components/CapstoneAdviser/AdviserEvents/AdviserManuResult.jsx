// src/components/CapstoneAdviser/AdviserEvents/AdviserManuResult.jsx
...
export default function AdviserManuResult() {
  ...
  const handleFileClick = async (sched) => {
    ...
    const { value: action } = await MySwal.fire({
      title: "File Options",
      text: sched.file_uploaded ? `Current File: ${sched.file_uploaded}` : "No file uploaded yet",
    ...
    if (action && canUpload) {
      ...
      input.onchange = async (e) => {
        ...
        try {
          ...
          setSchedules((prev) =>
            prev.map((s) =>
              s.id === sched.id ? { ...s, file_uploaded: cleanName, file_url: filePath } : s
            )
          );
  ...
  const updateField = async (rowId, field, value) => {
    setSchedules((prev) => prev.map((s) => (s.id === rowId ? { ...s, [field]: value } : s)));
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="px-6 pb-6 max-w-7xl w-full mx-auto flex-1">
                    ...
                    <tr key={sched.id}>
                      ...
                      <td>
                        <div className="dropdown-control-wrapper">
                          <select
                            value={sched.ai}
                            onChange={(e) => updateField(sched.id, "ai", parseInt(e.target.value))}
                            className="percentage-select"
                          >
                            {PERCENTAGE_OPTIONS.map((opt) => (
                              <option key={opt} value={opt}>
                      ...
                      <td>
                        <button className="file-button" onClick={() => handleFileClick(sched)}>
                      ...
                      <td>
                        <div className="dropdown-control-wrapper">
                          <select
                            ...
                            onChange={(e) => updateField(sched.id, "verdic", parseInt(e.target.value))}
        ...
        {/* Inline styles used in this screen */}
        <style>{`
          .table-scroll-area::-webkit-scrollbar { display: none; }
          .section-title { font-weight: 600; color: #3B0304; display: flex; align-items: center; gap: 8px; margin: 0; }
          .divider { height: 1.5px; background-color: #3B0304; width: calc(100% + 50px); margin-left: -16px; border-radius: 50px; margin-bottom: 1.5rem; border: none; }
          .search-input-container { position: relative; width: 100%; max-width: 250px; }
          .search-input { width: 100%; padding: 7px 12px 7px 35px; border: 1px solid #B2B2B2; border-radius: 6px; background-color: white; color: #3B0304; font-size: 0.85rem; box-shadow: none; transition: border-color 0.2s; height: 34px; }
          ...
          .tasks-table th { background-color: #f8f9fa !important; font-weight: 600 !important; color: #3B0304 !important; text-transform: uppercase; font-size: 0.75rem; padding: 12px 6px !important; white-space: nowrap; text-align: center; }
          .tasks-table td { padding: 8px 6px !important; font-size: 0.875rem; color: #495057; border-bottom: 1px solid #dee2e6; vertical-align: middle; text-align: center; }
          .tasks-table tbody tr:hover { background-color: #f8f9fa; }
          .dropdown-control-wrapper { position: relative; display: inline-flex; align-items: center; vertical-align: middle; min-width: 120px; }
          .dropdown-icon-chevron { position: absolute; right: 6px; pointer-events: none; font-size: 0.75rem; z-index: 2; }
          .percentage-select, .status-select { border: 1px solid #ccc !important; background-color: white !important; color: #3B0304 !important; border-radius: 4px !important; padding: 4px 20px 4px 6px !important; font-size: 0.85rem !important; appearance: none !important; cursor: pointer; width: 100%; }
          .percentage-select:focus, .status-select:focus { outline: 1px solid #3B0304; }
          .file-button { background: none; border: 1px solid #3B0304; color: #3B0304; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 0.85rem; transition: background-color 0.2s; }
          .file-button:hover { background-color: #f0f0f0; }
