...
const ManagerTaskView = ({ task, onBack }) => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setComments((prev) => prev.map((c) => (c.id === editingComment ? { ...c, text: editCommentText } : c)));
  ...
  return (
    <div className="container mt-4">
      ...
      <h2 className="text-lg font-bold" style={{ color: "#5a0d0e" }}>
        Manager Board &gt; {task.status} &gt; {task.assigned_to || task.group_name || task.task}
      </h2>
        ...
        <div className="tasks-layout">
          ...
          <div className="left-column">
            <div className="info-header">
              <div className="info-title">{task.task || task.group_name || "Untitled Task"}</div>
              <div className="info-status">
                <span className="status-dot" />
                <span className="status-text">{task.status || "No Status"}</span>
              </div>
            </div>

            <div className="info-grid">
              <div>Assigned To:</div><div>{task.assigned_to || "None"}</div>
              ...
              <div>Time:</div><div>{task.due_time || task.time || "None"}</div>
            </div>
          </div>
